...
export default function BandejaSolicitudExpandDatos({ sel, variant }) {
  ...
  // El jefe lee datos de negocio, no identificadores: sin monoespaciado ni corte por carácter.
  const esJefe = variant === "jefe";
  const valueClassName = esJefe
    ? "break-words text-sm font-medium text-slate-900 leading-snug"
    : "break-all font-mono text-[13px] text-slate-800 leading-snug";

  return (
    <dl className={esJefe ? "grid gap-2.5 text-sm" : "grid gap-2 text-sm"}>
