...
export default function BandejaJefeSolicitudDetalle({ sel, motivo, setMotivo, procesando, onDecidir }) {
  ...
  if (sel.puede_decidir !== true) {
    const cuando = formatInstanteBandeja(sel.jefe_revision_en);
    return (
      <div className="border-t border-blue-100 bg-blue-50/20 px-4 py-3">
        <p className="text-sm text-slate-700">
          {cuando ? (
            <>
              Tu decisión quedó registrada el <span className="font-medium text-slate-900">{cuando}</span>.
            </>
          ) : (
            "Trámite cerrado; no hay más acciones en esta bandeja."
          )}
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-4 border-t border-blue-100 bg-blue-50/30 px-4 py-4">
      ...
      <p className="text-xs leading-relaxed text-slate-500">
        Al aprobar, la solicitud queda aprobada; RRHH registra toma de conocimiento. Al rechazar, se anula y se
