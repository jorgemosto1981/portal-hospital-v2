...
import { callListarSolicitudesBandejaAuditorMedica } from "../../services/callables.js";

export const FILTROS_VISTA_AUDITOR = [
  { value: "completas", label: "Completas (listas para clasificar)" },
  { value: "provisorias", label: "Provisorias (sin certificado)" },
  { value: "todas", label: "Todas en pendiente médico" },
];

const PAGE_SIZE = 10;

export function useBandejaAuditorSolicitudes() {
  ...
  const [filtroVista, setFiltroVista] = useState("completas");
  const [dni, setDni] = useState("");
  const [usuario, setUsuario] = useState("");

  const [applied, setApplied] = useState({
    filtroVista: "completas",
    dni: "",
    usuario: "",
  });
  ...
  const fetchPage = useCallback(
    async ({ append = false, cursor = null, filtros = applied } = {}) => {
      ...
      const res = await callListarSolicitudesBandejaAuditorMedica(body);
