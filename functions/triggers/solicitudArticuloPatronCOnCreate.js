...
/**
 * Trigger: alta solicitud Patron C en BORRADOR -> valida via motor V2 y descuenta saldo global.
...
const { runPatronCAltaMotorV2, ANIO_ORIGEN_GLOBAL } = require("../modules/shared/patronCAltaMotorV2");
const {
...
const onSolicitudArticuloPatronCOnCreate = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (schemaVersion !== 3 && patron !== "C") return;
    ...
    let motor;
    try {
      motor = await runPatronCAltaMotorV2({
        db,
        solicitud: d,
        excludeSolId: solId,
        authToken: null,
      ...
      logger.error("solicitud_patron_c_motor_v2_error", {
        solId,
    ...
    try {
      ...
      logger.error("solicitud_patron_c_autorizacion_error", {
        solId,
        message: err instanceof Error ? err.message : String(err),
      });
      await solRef.update({
    ...
    const cantidadConsumo = motor.cantidad_consumo;
    ...
    try {
      await db.runTransaction(async (tx) => {
        ...
        if (!salSnap.exists) {
          tx.update(solRef, {
            ...motorOkPayload,
            estado_solicitud_id: ESTADO_SOLICITUD_RECHAZADA,
            motor_codigos: ["SALDO_GLOBAL"],
            motor_mensajes: ["No hay saldo global disponible."],
          });
          return;
        }

        const match = pickBolsaParaConsumo(salSnap.data() || {}, motor.articulo_id, ANIO_ORIGEN_GLOBAL);
        if (!match) {
          tx.update(solRef, {
            ...motorOkPayload,
            estado_solicitud_id: ESTADO_SOLICITUD_RECHAZADA,
            motor_codigos: ["SALDO_GLOBAL"],
            motor_mensajes: ["No hay bolsa global para este articulo."],
          });
          return;
        }
        ...
        if (!Number.isFinite(disp) || disp < cantidadConsumo) {
          tx.update(solRef, {
            estado_solicitud_id: ESTADO_SOLICITUD_RECHAZADA,
            motor_codigos: ["SALDO_GLOBAL"],
            motor_mensajes: [`Saldo insuficiente: disponible ${disp}, solicitado ${cantidadConsumo}.`],
            motor_validado_en: FieldValue.serverTimestamp(),
            actualizado_en: FieldValue.serverTimestamp(),
          });
          return;
        ...
        const debito = {
          ...
          unidad: motor.unidad_consumo || "horas",
        };

        tx.update(salRef, {
          [`bolsas.${match.bolsaId}.consumido`]: cons + cantidadConsumo,
          [`bolsas.${match.bolsaId}.disponible`]: disp - cantidadConsumo,
        ...
        tx.update(solRef, {
          ...
          motor_cantidad_descontada: cantidadConsumo,
          motor_unidad_consumo: motor.unidad_consumo || "horas",
          motor_snapshot: motor.motor_snapshot || null,
      ...
      logger.error("solicitud_patron_c_tx_error", { solId, message: err instanceof Error ? err.message : String(err) });
      await solRef.update({
        estado_solicitud_id: ESTADO_SOLICITUD_RECHAZADA,
    ...
    try {
      ...
      logger.warn("solicitud_patron_c_mdc_emit", {
        solId,
...
module.exports = { onSolicitudArticuloPatronCOnCreate };
