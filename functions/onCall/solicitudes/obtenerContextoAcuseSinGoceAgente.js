...
const obtenerContextoAcuseSinGoceAgenteCallable = onCall(async (request) => {
  if (!request.auth) {
  ...
  const result = await obtenerContextoAcuseSinGoceAgente(db, solicitudId, titularPersonaId);
...
module.exports = {
  obtenerContextoAcuseSinGoceAgente: obtenerContextoAcuseSinGoceAgenteCallable,
};
