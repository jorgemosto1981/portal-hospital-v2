...
/**
 * Config resolver Patron C — cableado total de los 7 bloques del configurador.
 *
 * Patron C = cuenta corriente continua (cfg_rcc_nunca + origen externo).
 * Saldo global, sin reinicio ciclico, unidad tipica: horas.
 *
 ...
 *
 * @see docs/v2/RFC_MOTOR_V2_AS_BUILT.md
 */

const { resolvePatronSaldo, PATRON_SALDO_C } = require("./resolvePatronSaldo");

/**
 * Assert que la version es Patron C.
 * @param {object} versionData
 */
function assertPatronSaldoC(versionData) {
  ...
  if (patron !== PATRON_SALDO_C) {
    const err = new Error(`Patron saldo esperado C, obtenido: ${patron || "null"}`);
    err.code = "ERROR_PATRON_SALDO_NO_C";
    throw err;
  }
  return patron;
}
...
/**
 * Resuelve config completa de los 7 bloques para motor Patron C.
 * @param {object} versionData
 * @returns {object}
 */
function resolvePatronCMotorConfig(versionData) {
  assertPatronSaldoC(versionData);
 ...
 * @param {object} cfg — resultado de resolvePatronCMotorConfig
 * @param {string} versionId
 */
function buildPatronCConfigUsada(cfg, versionId) {
  return {
    version_aplicada_id: versionId || null,
    motor_tipo: "patron-c-v2",
    ...
    regla_computo_horas_id: cfg.regla_computo_horas_id,
    ...
    fraccionamiento_habilitado: cfg.fraccionamiento_habilitado,
    modulo_fraccionamiento_minutos: cfg.modulo_fraccionamiento_minutos,
...
module.exports = {
  assertPatronSaldoC,
  resolvePatronCMotorConfig,
  buildPatronCConfigUsada,
};
