...
const { TOPE_MOVIMIENTOS_MAX } = require("./topeMovimientosConfig");
 ...
 * @param {string} raw
 */
function canonicalizarSegmentoTope(raw) {
 ...
 */
function overrideCuentaPorVigencia(creadoEn, vigenteDesde) {
 ...
 */
function esTrasladoReemplazoV2Override(ov) {
 ...
 * @param {Record<string, unknown>} ov
 */
function esIntercambioGuardiaV2Override(ov) {
 ...
 * @param {Record<string, unknown>} ov
 */
function overrideCuentaParaTope(ov) {
 ...
 */
function derivarIncrementosDesdeOverridePersistido(ov, personaId, fechaYmd, gdt) {
 ...
 */
function claveIncrementoTope(inc) {
 ...
 */
function derivarIncrementosTopeDesdeBatchItems(items) {
 ...
 * @param {string | null | undefined} vigenteDesde
 */
function mapaConteoHistoricoTope(overridesEnriquecidos, vigenteDesde) {
 ...
 * }} params
 */
function evaluarTopeMovimientosBatch(params) {
  const {
 ...
 * @param {string} [nowIso]
 */
function topeMovimientosActivo(vigenteDesde, nowIso = new Date().toISOString()) {
  const desde = String(vigenteDesde || "").trim();
  if (!desde) return false;
 ...
 * }} params
 */
function contarMovimientosTramoDia(params) {
  const {
    overridesMes,
    persona_id,
    fecha_ymd,
...
module.exports = { canonicalizarSegmentoTope, overrideCuentaPorVigencia, esTrasladoReemplazoV2Override, esIntercambioGuardiaV2Override, overrideCuentaParaTope, derivarIncrementosDesdeOverridePersistido, claveIncrementoTope, derivarIncrementosTopeDesdeBatchItems, mapaConteoHistoricoTope, evaluarTopeMovimientosBatch, topeMovimientosActivo, contarMovimientosTramoDia };
