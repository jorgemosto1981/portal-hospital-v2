...
/**
 * Anclaje por día civil en {@link ZONA_HORARIA_INSTITUCIONAL} (ver shared/utils/antiguedadCalculator.js).
...
function toUtcDay(value, fieldName) {
  ...
  if (value instanceof Date) {
    if (Number.isNaN(value.getTime())) throw new Error(`Fecha inválida en ${fieldName}.`);
  ...
  if (/^\d{4}-\d{2}-\d{2}T/i.test(raw) || /^\d{4}-\d{2}-\d{2}\s/.test(raw)) {
    ...
    if (Number.isNaN(ms)) throw new Error(`Formato de fecha/hora inválido en ${fieldName}.`);
    const { year, month, day } = ymdEnZonaDesdeInstante(ms);
    return civilDateInZonaToUtcAnchorMs(year, month, day);
  }

  const match = raw.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) throw new Error(`Formato de fecha inválido en ${fieldName}. Use YYYY-MM-DD o ISO-8601 con hora.`);
