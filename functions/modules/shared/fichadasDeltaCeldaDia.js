...
const FIRESTORE_BATCH_SAFE_MAX = 400;
 ...
 * @returns {string[]}
 */
function normalizarAdvertenciasFichadaAbiertas(raw) {
  if (!Array.isArray(raw)) return [];
  const set = new Set(
    raw
      .map((c) => String(c || "").trim())
 ...
 * @returns {T[][]}
 */
function segmentarOperacionesFirestore(operaciones, maxPorBatch = FIRESTORE_BATCH_SAFE_MAX) {
...
module.exports = { FIRESTORE_BATCH_SAFE_MAX, normalizarAdvertenciasFichadaAbiertas, evaluarDeltaCeldaDia, segmentarOperacionesFirestore };
