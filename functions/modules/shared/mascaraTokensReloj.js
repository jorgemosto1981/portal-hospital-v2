...
/**
 * Extracción de campos TXT según máscara de tokens (cfg_reloj_biometrico.mascara_tokens).
 */

const MASCARA_RELOJ_DEFAULT = "TTTTT DD/MM/YY HH:MM RRR CC";
...
function normalizarMascaraTokens(mascara) {
  return String(mascara || "").replace(/\s+/g, " ").trim();
}

function esMascaraParserV1(mascara) {
  const n = normalizarMascaraTokens(mascara);
  return !n || n === MASCARA_RELOJ_DEFAULT;
}

/**
 * @param {string} mascara
 */
function segmentarMascaraTokens(mascara) {
 ...
 * @param {string} mascara
 */
function extraerCamposSegunMascara(linea, mascara) {
  ...
  for (const seg of segmentos) {
    if (seg.tipo === "literal") {
      if (raw[pos] !== seg.literal) {
        errores.push(`Separador esperado «${seg.literal}» en posición ${pos + 1}.`);
        break;
      }
      pos += 1;
      continue;
    ...
    if (seg.tipo === "tarjeta" || seg.tipo === "reloj" || seg.tipo === "codigo_funcion") {
      ...
      if (trozo.length < (seg.ancho || 0)) {
        errores.push(`Faltan caracteres para ${seg.tipo}.`);
        break;
      }
      pos += seg.ancho || 0;
    ...
    if (seg.tipo === "fecha_iso") {
      ...
      if (!ymd) {
        errores.push("Fecha ISO inválida.");
        break;
      }
  ...
  if (!errores.length && pos < raw.length) {
    ...
    if (resto) errores.push(`Sobran caracteres al final: «${resto}».`);
  }

  const ok =
    errores.length === 0 &&
    Boolean(campos.numero_tarjeta) &&
    Boolean(campos.fecha_ymd) &&
    Boolean(campos.hora_hm);

  return { ok, campos, errores, consumido: pos };
}

module.exports = { MASCARA_RELOJ_DEFAULT, normalizarMascaraTokens, esMascaraParserV1, segmentarMascaraTokens, extraerCamposSegunMascara };
