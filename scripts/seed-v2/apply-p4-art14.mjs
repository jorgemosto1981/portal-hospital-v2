/**
 * Seed P4.3 — Art. 14 licencia médica corta (cfg_articulos + versión publicada).
 *
 * Uso (raíz repo):
 *   node scripts/seed-v2/apply-p4-art14.mjs --dry-run
 *   node scripts/seed-v2/apply-p4-art14.mjs --apply
...
import { buildP4LicenciaMedicaDocuments } from "./lib/buildP4LicenciaMedicaVersion.mjs";
...
const SPECS_PATH = join(repoRoot, "docs/v2/seeds/p4_art14/ART14_P43_SPECS.json");
const APPLIED_PATH = join(repoRoot, "docs/v2/seeds/p4_art14/applied-ids.json");
...
for (const entry of specRoot.articulos) {
  ...
  const built = buildP4LicenciaMedicaDocuments(merged, ids);
  const errors = [];
...
for (const entry of specRoot.articulos) {
  ...
  const merged = mergeSpec(entry);
  const built = buildP4LicenciaMedicaDocuments(merged, {
    artId: mapped.artId,
    verId: mapped.verId,
  });
    ...
    if (REAPPLY) {
      ...
      batch.set(
        coreRef,
        {
          ...built.core,
          seed_p4_art14: true,
          seed_p4_art14_reaplicado_en: FieldValue.serverTimestamp(),
  ...
  batch.set(coreRef, {
    ...built.core,
    seed_p4_art14: true,
    seed_p4_art14_aplicado_en: FieldValue.serverTimestamp(),
    ayuda_agente_ui: entry.ayuda_agente_ui || null,
  });
  batch.set(verRef, {
    ...built.version,
