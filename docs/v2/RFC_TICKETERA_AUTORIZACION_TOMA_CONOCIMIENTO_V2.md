...
| 2 | Quién autoriza | En el `grupo_trabajo_id` ancla: integrantes con `nivel_jerarquico` **estrictamente mayor** que el titular (escala **01 = menor**, **99 = mayor**). Entre superiores, quien(es) tengan el **nivel más alto** (hasta 99); empate OR en ese nivel. **Un solo paso** de autorización. |
...
6. nivel_auth = MAX(nivel) sobre candidatos (mayor jerarquía en la burbuja)
