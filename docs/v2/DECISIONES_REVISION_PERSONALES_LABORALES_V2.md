...
**Nota 27/04/2026 — catálogo de efectores y panel (una sola fuente de verdad):** el criterio del acuerdo **A2** (entidad “efector” distinta de “grupo de trabajo”; **dos** FK en `hlc_*` + `grupo_de_trabajo_id`) **no cambia**. Lo que se unifica es el **nombre de colección** y el **contenedor en Firestore** para el ABM: el catálogo vive en **`cfg_efectores`**, y `efector_designacion_id` / `efector_cumplimiento_id` apuntan al **id de documento** allí. La antigua redacción “colección `efectores` con id `efe_*`” se considera **obsoleta** en V2; no se reabre A2, se **documenta el destino** en [`MODULO_DATOS_LABORALES_V2.md`](./MODULO_DATOS_LABORALES_V2.md) §4.2, plan maestro §B y [`RULEBOOK_V2.md`](./RULEBOOK_V2.md).
...
| **A2** | Grupo de trabajo vs efector | **Cerrado (chat):** **no** un solo `grupos` para ambos. **(1) `grupos_de_trabajo`:** organigrama, burbujeo, ticket; id **`gdt_<ULID>`**. **(2) Catálogo de efectores (implementación V2: colección `cfg_efectores`):** **seleccionable y configurable** por panel/ABM; **ids de documento** estables (p. ej. `CFG_EFE_*` o `efe_<ULID>` según Rulebook). *Redacción histórica: `efectores` / `efe_*` — ver nota 27/04/2026.* **(3) `historial_laboral_cargos`:** **dos** FK a documentos de **`cfg_efectores`** (campos `efector_designacion_id`, `efector_cumplimiento_id`) y **una** a **`grupos_de_trabajo`**. — en [`PLAN_DESARROLLO_VERSION2.md`](../../PLAN_DESARROLLO_VERSION2.md) §B y [`MODULO_DATOS_LABORALES_V2.md`](./MODULO_DATOS_LABORALES_V2.md) §4. |
| **A3** | Desalineación módulo vs `PLAN_DESARROLLO_VERSION2` | **Precedencia:** 1) módulos `docs/v2/MODULO_*` y este archivo; 2) [`REVISION_ALINEACION_PLAN_V2.md`](./REVISION_ALINEACION_PLAN_V2.md); 3) plan maestro. Tras el acuerdo **A2 (abril 2026)**, el plan maestro **§B y §BD** y el módulo laboral quedan **alineados a `grupos_de_trabajo` (`gdt_*`) y al catálogo de efectores en `cfg_efectores`**; la redacción con una sola colección `grupos` es **reemplazada** en documentos actualizados. Cualquier texto residual aún con `grupos` / `grp_*` unificado = **migrar al cerrar** esa revisión. |
...
| ID | Tema | Definición acordada (propuesta) |
|----|------|----------------------------------|
| **C1** | Campos de enlace en `hlc_*` (trabajo vs efector) | **Cerrado (chat 23/04):** **confirmado** — mismo criterio **A2**: `grupo_de_trabajo_id` → `gdt_*`; `efector_designacion_id` y `efector_cumplimiento_id` → **id de documento** en **`cfg_efectores`**. *Redacción antigua `efe_*` = legacy.* Ver [`MODULO_DATOS_LABORALES_V2.md`](./MODULO_DATOS_LABORALES_V2.md) §4.3 y plan maestro §B. |
...
| **C10** | Nivel de jerarquía y carga **por grupo de trabajo** (`hlg_*`) | **Cerrado (chat 23/04), actual. doc 27/04/2026:** por cada vínculo del **usuario** (`persona_id`) a un **grupo de trabajo** concreto (fila en **`historial_laboral_grupos`**, `hlg_*`) deben definirse: **vigencia** (`fecha_inicio` / `fecha_fin`); **`nivel_jerarquico`** entero **1–99** (nivel en esa **burbuja**; **no** hay colección `cfg_nivel_jerarquia`); y **`carga_por_dia_semana`** (lista de pares `dia_semana_id` → `cfg_dia_semana` + `horas` numéricas) para desagregar la jornada **día a día**. Insumo de **burbujeo, Ticket, informes y reglas**. El **`nivel_jerarquico_numero` opcional en `hlc_*`** no sustituye al nivel **por grupo**; si coexisten, predomina el de `hlg_*` para organigrama. **Coherencia:** suma semanal `hlg_*` vs `carga_horaria_total` de `hlc_*` (Callable, tolerancia según reglas). |
| **C5** | Efector (cumplimiento) en validaciones y RFC fusión con organigrama | **Cerrado (chat 23/04): (1) Uso operativo:** **`efector_cumplimiento_id`** (y en general las **FK a documentos de `cfg_efectores` en** `hlc_*`) deben ser **tenidas en cuenta** en **validaciones** y **filtrados** posteriores (**por id**), p. ej. listados, reglas y cruces con Ticket — **no** son solo descriptivos. **(2) RFC “un solo nodo” efector+organigrama:** sigue **fuera de alcance** V2 inicial; si el hospital lo pide, **nuevo RFC**; el contrato de **tres FK** (A2) se mantiene. |
...
| **C9** | Unificación con doc **Ticket** (otra PC) | **Pendiente (chat 23/04):** criterio de cierre *aún no fijado*. **Guía** sin cambio: al fusionar ramas/docs, alinear nombres (`hlc_*`, `gdt_*`, catálogo **efectores** en **`cfg_efectores`**); checklist [`UNIFICACION_OTRA_PC_Y_TICKET.md`](./UNIFICACION_OTRA_PC_Y_TICKET.md) y §9 módulo laboral. Registrar cierre en changelog cuando se defina. |
...
| **D4** | Ticket lee laborales | **Cerrado (chat 23/04):** mínimo de **lectura** para resolver pantallas/labels: `persona_id`, `hlc_*` **vigentes**, nombres vía **`gdt_*`**, labels/efectores vía **`cfg_efectores`**, demás `cfg_*`; **sin** jefe en `hlc_*`; jerarquía = lógica Ticket. **No** se exige que Ticket **filtre** por `efector_cumplimiento_id` — el criterio de asignación/filtrado alineado al producto es por **`grupos_de_trabajo`** asignados o relevantes al usuario. *Otras reglas/ módulos pueden seguir usando efector (ver **C5**).* |
...
| 2026-04-27 | **Nota 27/04/2026** bajo el encabezado: catálogo de efectores = **una SSoT** en **`cfg_efectores`**, FK en `hlc_*` a **id de documento**; `efectores` / `efe_*` como nombres legacy. **A2** y **A3** ajustados en texto; **C1, C5, C9, D4** alineados; semántica de A2 **sin reabrir** la decisión. |
| 2026-04-27 | **C10** re-redactada: **sin** `cfg_nivel_jerarquia` — **`hlg_*.nivel_jerarquico` (1–99)**; criterio de burbuja + carga por día inalterable en lo sustancial. |
