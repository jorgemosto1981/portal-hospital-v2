...
1. Tras el **paso B** (y en todo ingreso en que haya que refrescar), el backend asigna en el token (vía `setCustomUserClaims` o equivalente) al menos:
   ...
   - **Perfil laboral (V2, operativo):** se escriben **`roles_hlc_vigentes`** (array de FK `cfg_rol` desde **HLc** en cadenas vigentes HLc→HLd→HLg), **`cargo_activo`**, **`labor_rol_conflicto`**. **`portal_role`** y **`perfil_rol_id`** en claims quedaron **deprecados** (se escriben `null`; ver [`RFC_ACCESO_ROLES_HLC_MENUS_V2.md`](./RFC_ACCESO_ROLES_HLC_MENUS_V2.md)). La lógica vive en `functions/modules/shared/laborProfile.js` y `authClaims.js`. **`applyLaborAwareSessionClaims`** se invoca en: **`syncSessionClaims`**, **`registrarPrimerAcceso`**, **`vincularCuentaConDni`**, **`onboardingMvpCompletar`**, y **`refreshSessionClaimsForPersona`** tras guardado laboral (`guardarRegistroLaboralTemporal`, `rrhhDeshabilitarHlc`).
2. **`usuarios_cuenta.role_ids`:** el alta RRHH ya **no** lo rellena por defecto; el rol de aplicación canónico es **`historial_laboral_cargos.rol_id`**. Materializar `role_ids` en claims sigue siendo *opcional* / legado; no sustituye al modelo HLc.
...
| 2026-05-03 | **§3.3** claims laborales (`perfil_rol_id`, `cargo_activo`, … desde HLc); **`applyLaborAwareSessionClaims`** unifica `syncSessionClaims`, registro primer acceso, vínculo DNI y fin onboarding; rol canónico en **HLc**, no `role_ids` en alta. |
| 2026-05-19 | **§3.3** actualizado: claim canónico **`roles_hlc_vigentes[]`**; deprecación escritura `portal_role` / `perfil_rol_id`; refresh claims post-guardado laboral. Ver [`RFC_ACCESO_ROLES_HLC_MENUS_V2.md`](./RFC_ACCESO_ROLES_HLC_MENUS_V2.md). |
