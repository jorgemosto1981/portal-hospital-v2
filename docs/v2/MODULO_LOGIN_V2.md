...
1. Tras el **paso B** (y en todo ingreso en que haya que refrescar), el backend asigna en el token (vía `setCustomUserClaims` o equivalente) al menos:
   ...
   - **Perfil laboral (V2, operativo):** también se escriben **`perfil_rol_id`** (FK `cfg_rol` deducido desde **HLc** en cadena vigente HLc→HLd→HLg), **`cargo_activo`**, **`labor_rol_conflicto`** y **`portal_role`** (derivado; p. ej. acceso panel RRHH cuando el rol en HLc es `CFG_RRHH`). La lógica vive en `functions/modules/shared/laborProfile.js`. La misma composición de claims se aplica mediante **`applyLaborAwareSessionClaims`** (`functions/modules/shared/authClaims.js`) en: Callable **`syncSessionClaims`**, **`registrarPrimerAcceso`**, **`vincularCuentaConDni`** y **`onboardingMvpCompletar`**, para no dejar el token sin datos laborales tras vínculo u onboarding.
