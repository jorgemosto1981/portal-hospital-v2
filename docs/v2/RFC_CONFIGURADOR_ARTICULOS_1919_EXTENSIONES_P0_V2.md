...
### 2.5 Ciclo de vida ABM — vigencia y deshabilitación

**Requisito RRHH (épica 1919):** crear, **modificar desde fecha X** (nueva versión publicada), **deshabilitar desde fecha X** (sin delete). Contrato: [`MODULO_CONFIGURACION_ARTICULOS_V2.md`](./MODULO_CONFIGURACION_ARTICULOS_V2.md) §5.1.

| Capacidad | Schema (existente / a cerrar) | Gap UI |
|-----------|----------------------------------|--------|
| Alta artículo + versión borrador | `cfg_articulos`, `versiones` | Parcial — flujo crear/listar |
| Publicar versión | `estado_version_id`, `version_actual_id` | OK guardado/publicar |
| `vigente_desde` / `vigente_hasta` en núcleo o versión | Campos en schema producto | **Exponer en ABM** + validación Zod |
| Deshabilitar desde X | `activo`, `estado_articulo_id`, `vigente_hasta` | **Acción dedicada** con fecha (no solo checkbox) |
| Modificar desde X | Nueva `ver_*` publicada; solicitudes antiguas con snapshot | Wizard “nueva versión vigente desde” |
| Listado RRHH | Filtro vigentes / obsoletos / próximos a vencer | Mejora listado configuración |

**Motores / ticketera:** al listar artículos ingreso, respetar **ventana de vigencia** + `activo` + `estado_articulo_id` (sin hardcode por `art_*`).

**Prioridad:** **P5** junto a `opciones_consumo_solicitud[]`; **bloqueante** para operación masiva 1919 si RRHH depende de fechar cambios normativos.

**Excepción LAO (Art. 40):** no cambiar el esquema de **versiones por `correspondencia_anio`** ni Patrón A / check-in; las mejoras ABM genéricas (§2.5) se aplican a LAO solo donde ya existen (`publicar`, grilla de versiones, `activo`). Ver [`MODULO_CONFIGURACION_ARTICULOS_V2.md`](./MODULO_CONFIGURACION_ARTICULOS_V2.md) §5.2.
