...
6. **Subnivel por grupo (`hlg_*`):** cada **asignación** del agente a un **grupo de trabajo** (`gdt_*`) concreto lleva **vigencia propia**, **`nivel_jerarquico`** (número **1–99**, jerarquía en esa burbuja; **no** es catálogo `cfg_*`) y **régimen horario obligatorio** (`regimen_horario_id` → `cfg_regimen_horario`); la carga semanal se deriva dinámicamente del régimen — **C10**; contrato **§4.4–4.5**.
...
| `regimen_horario_id` | string | **[O]** | FK **`cfg_regimen_horario`**. Define el régimen horario del agente en este grupo; la carga semanal se **deriva dinámicamente** del documento del régimen (fijo: suma `horas_efectivas` de `dias[]`; rotativo: promedio de `ciclo[]` x 7; planificado: `carga_horaria_semanal_teorica`). **Campo `carga_por_dia_semana` eliminado (mayo 2026):** era redundante. |
| `regimen_fecha_ancla` | Timestamp \| null | **[C]** | Día 1 del ciclo para regímenes rotativos. Para fijos/planificados, dejar vacío. |
| `activo` | boolean | **[O]** | |
| `creado_en` / `actualizado_en` | Timestamp | **[O]** | |
| `schema_version` | number | **[O]** | |
...
- La **carga semanal derivada** del `regimen_horario_id` de cada `hlg_*` debe ser **reconciliable** con `carga_horaria_total` de `hlc_*` (warning informativo en Callable, sin bloquear). `regimen_horario_id` es **obligatorio** (mayo 2026).
...
### 4.5 Derivación de carga semanal desde `cfg_regimen_horario` *(mayo 2026)*

El campo **`carga_por_dia_semana`** fue **eliminado** de `hlg_*`. La carga semanal se **deriva dinámicamente** del documento `cfg_regimen_horario` referenciado por `regimen_horario_id` (obligatorio):

| Tipo de patrón | Derivación |
|----------------|-----------|
| **fijo** | Suma de `dias[].turno.horas_efectivas` (7 días; francos y no laborables = 0). |
| **rotativo** | `(suma de ciclo[].turno.horas_efectivas / ciclo.length) * 7` (promedio semanal). |
| **planificado** | Campo `carga_horaria_semanal_teorica` del documento régimen. |

La función `derivarCargaSemanalDesdeRegimen(regimenDoc)` implementa esta lógica tanto en backend (`catalogosShared.js`) como en frontend (`utils.js`).

#### 4.5.1 Reconciliación con `hlc_*.carga_horaria_total`

- **`carga_horaria_total`** en `hlc_*` se interpreta como **carga referencial en horas por semana** del cargo.
- Para cada `hlg_*` se deriva la carga semanal desde su `regimen_horario_id` mediante `derivarCargaSemanalDesdeRegimen()`.
- La **suma de cargas derivadas** de todos los `hlg_*` bajo el mismo `hlc_*` debe ser reconciliable con `carga_horaria_total` (**ε = 0,01**).
- Si hay desvío, el sistema genera un **warning informativo** (código `VAL-HLG-W003`) sin bloquear el guardado.

#### 4.5.2 Ejemplo de derivación (referencia)

Un régimen **fijo** con 5 días laborables de 8 h y 2 francos genera `carga_semanal = 40 h`. Si el `hlc_*` tiene `carga_horaria_total = 40`, la reconciliación pasa. Un régimen **rotativo** de ciclo 4 (3 laborables de 12 h + 1 franco) genera `(36/4)*7 = 63 h` semanales teóricas.
...
| **`cfg_regimen_horario`** | `hlg_*.regimen_horario_id` (régimen horario obligatorio por grupo; carga semanal derivada dinámicamente). |
...
| 2026-05-26 | **Deprecación de `carga_por_dia_semana`:** campo eliminado de `hlg_*`; `regimen_horario_id` pasa a ser **obligatorio**; carga semanal se **deriva dinámicamente** del documento `cfg_regimen_horario` mediante `derivarCargaSemanalDesdeRegimen()`. Reconciliación adaptada (warning informativo `VAL-HLG-W003`). §4.4.1, §4.5, §6 actualizados; `cfg_dia_semana` ya no es consumido por HLG. |
