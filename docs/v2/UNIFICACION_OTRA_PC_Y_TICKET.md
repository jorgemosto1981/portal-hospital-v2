...
- **Contenido publicado en `origin/refactorizacion-fase1`:** documentación V2: unificación (este archivo), datos laborales (`grupos_de_trabajo` / `cfg_efectores` / `hlc_*`, horas), configuración (`vigente_desde` / `vigente_hasta` en `cfg_*`), Rulebook, cuestiones Ticket/roles/estados, README y PLAN. **No** fijar aquí el hash del tip; el remoto es la fuente de verdad.
...
|------|---------------------|------------|
| Efectores y grupos de trabajo | Catálogo **`cfg_efectores`** (reemplaza la fila bajo el nombre de colección `efectores`); organigrama **`grupos_de_trabajo`** (`gdt_*`); en `hlc_*`: `efector_designacion_id`, `efector_cumplimiento_id`, `grupo_de_trabajo_id`; marca **`es_efector_institucional`**. *Decisión **A2** (misma en sustancia).* | [`MODULO_DATOS_LABORALES_V2.md`](./MODULO_DATOS_LABORALES_V2.md) §4.1–4.3; plan maestro §B; [`DECISIONES_REVISION_PERSONALES_LABORALES_V2.md`](./DECISIONES_REVISION_PERSONALES_LABORALES_V2.md) |
...
2. **Compatibilidad hacia atrás:** si otra rama usaba **una** colección `grupos` o solo catálogos bajo el nombre suelto `efectores`, migrar a **`gdt_*` + `cfg_efectores`** (tabla de equivalencia; **no** reutilizar un id con otro significado). Ver plan maestro §B.
...
5. **Ticket consume laborales:** **`persona_id`** + `hlc_*` vigentes + joins a **`gdt_*`**, **`cfg_efectores`**, resto de `cfg_*`; no duplicar SSoT del cargo en Ticket salvo snapshot acordado.
...
| Código con `grupos` / `grp_*` unificado | Colección o prefijos distintos a `gdt_*`/`cfg_efectores` | Capa repositorio + script de split/migración según plan maestro §B |
...
- [ ] **Seeds:** orden sugerido (config con vigencias → `gdt_*` / `cfg_efectores` → `hlc_*`) y ningún catálogo sin `vigente_desde` / `activo` en entornos reales.
...
| 2026-04-27 | Sustitución documental: catálogo canónico = **`cfg_efectores`** (panel/ABM; sustituye colección suelta `efectores` en criterio V2). |
